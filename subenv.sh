...
on_exit() {
    ...
    if [ ${exit_code} -ne 0 ]; then
        echo "helm-subenv install hook failed. Please remove the plugin using 'helm plugin remove subenv' and install again." > /dev/stderr
    fi
    exit ${exit_code}
}
trap on_exit EXIT
...
if [ "$(uname)" == "Darwin" ]; then 
    binary_url="https://github.com/hydeenoble/helm-subenv/releases/download/v${version}/helm-subenv_${version}_darwin_amd64.tar.gz"
elif [ "$(uname)" == "Linux" ] ; then
    binary_url="https://github.com/hydeenoble/helm-subenv/releases/download/v${version}/helm-subenv_${version}_linux_amd64.tar.gz"
fi
...
checksum_url="https://github.com/hydeenoble/helm-subenv/releases/download/v${version}/helm-subenv_${version}_checksums.txt"
